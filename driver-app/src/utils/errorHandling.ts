// Error handling utility for driver app
...
export const parseApiError = (error: any): string => {
  ...
  switch (status) {
    ...
    default:
      const errorMsg = typeof data?.error === 'object' && data?.error !== null ? (data.error as ApiError).message : undefined;
      return errorMsg || data?.message || `Server error (${status}). Please try again.`;
  }
};
...
const parseValidationError = (data: ApiErrorResponse): string => {
  ...
  if (data.error && typeof data.error === 'object') {
    ...
    switch (error.type) {
      ...
      case 'ORGANIZATION_REQUIRED':
        return 'Organization selection is required for driver registration. Please select your organization.';
      default:
  ...
  if (typeof data.error === 'string') {
    ...
    if (data.error.includes('Organization ID is required')) {
      return 'Please select your organization before registering as a driver.';
    }
  ...
  if (data.error && typeof data.error === 'object') {
    ...
    switch (error.type) {
      ...
      case 'ACCOUNT_INACTIVE':
        return 'Your account is inactive. Please contact your organization administrator.';
  ...
  if (data.error && typeof data.error === 'object') {
    ...
    switch (error.type) {
      case 'CONTEXT_MISMATCH':
        return 'This account cannot be used with the driver app. Please use the passenger app or contact your organization.';
      case 'INSUFFICIENT_PERMISSIONS':
        return 'You do not have permission to perform this action. Please contact your organization administrator.';
      case 'ROLE_MISMATCH':
        return 'Your account role does not match the driver app. Please contact your organization administrator.';
      default:
        return error.message || 'Access denied. Please contact your organization administrator.';
    }
  }

  return 'Access denied. Please contact your organization administrator.';
};
...
const parseServerError = (data: ApiErrorResponse): string => {
  if (data.error && typeof data.error === 'object') {
    return 'Server error occurred. Please try again later or contact your organization administrator.';
  }
 ...
 */
export const getContextualErrorMessage = (error: any, context: 'login' | 'register' | 'profile' | 'general' | 'send OTP' | 'verify OTP' = 'general'): string => {
  // First check for direct error message from response
  const responseError = error.response?.data?.error || error.response?.data?.message;
  
  // Handle OTP-specific errors directly
  if (context === 'verify OTP' || context === 'send OTP') {
    if (responseError) {
      // Return the server error message directly for OTP operations
      if (typeof responseError === 'string') {
        return responseError;
      }
      if (typeof responseError === 'object' && responseError !== null && 'message' in responseError) {
        return (responseError as ApiError).message;
      }
    }
  ...
  // Add context-specific guidance for driver app
  switch (context) {
    case 'login':
      if (baseMessage.includes('Invalid email or password')) {
        return baseMessage + ' If you forgot your password, please contact your organization administrator.';
      }
      if (baseMessage.includes('passenger app')) {
        return 'This account is for passengers only. Please use the passenger app or register as a driver.';
      }
      break;
    case 'register':
      ...
      if (baseMessage.includes('Organization')) {
        return baseMessage + ' Please select your organization from the list.';
      }
      break;
    case 'profile':
      ...
      break;
    case 'verify OTP':
      // Already handled above, but add fallback
      break;
    case 'send OTP':
      // Already handled above, but add fallback
      break;
