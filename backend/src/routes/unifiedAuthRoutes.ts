 ...
 */
// ============================================================================
// UNIFIED WEB LOGIN (Auto-detect user type)
// ============================================================================
 ...
 *   post:
 *     tags: [Web Dashboard Authentication]
 *     summary: Unified login for web dashboard (auto-detects user type)
 *     description: |
 *       Authenticates users for web dashboard access by automatically detecting their user type.
 *       Tries authentication in order: Super Admin -> Admin -> Organization.
 *       Returns the user with their detected role and appropriate tokens.
 *       
 *       **Security Note:** This endpoint does not expose role selection to the client.
 *       The user type is determined server-side based on credentials.
 *     security: []
 *     parameters:
 *       - in: header
 *         name: X-App-Context
...
router.post('/web/login',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const superAdminResult = await authService.authenticateSuperAdmin(email, password);
      if (superAdminResult.success) {
        return res.json({
          message: 'Login successful',
          user: superAdminResult.user,
          tokens: superAdminResult.tokens
        });
      }

      // Try Admin next
      const adminResult = await authService.authenticateAdmin(email, password);
      if (adminResult.success) {
        return res.json({
          message: 'Login successful',
          user: adminResult.user,
          tokens: adminResult.tokens
        });
      }

      // Try Organization last
      const orgResult = await authService.authenticateOrganization(email, password);
      if (orgResult.success) {
        return res.json({
          message: 'Login successful',
          user: orgResult.user,
          tokens: orgResult.tokens
        });
      }

      // All authentication attempts failed
      // Return generic error to not reveal which user types exist
