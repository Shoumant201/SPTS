...
export const AdminManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingAdmin) {
        ...
        await superAdminApi.createAdmin(createData);
  ...
  const handleToggleStatus = async (admin: Admin) => {
    try {
      setError('');
      await superAdminApi.updateAdmin(admin.id, { isActive: !admin.isActive });
          ...
          <tbody className="divide-y divide-gray-200">
            {admins.map((admin) => (
              <tr key={admin.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-gray-600">{admin._count?.organizations || 0}</td>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm">
            ...
            <p className="text-gray-600 mb-4">Are you sure you want to delete this admin? This action cannot be undone.</p>
