...
export const phoneAuthApi = {
  ...
  sendOtp: async (data: SendOtpRequest): Promise<SendOtpResponse> => {
    try {
      // Ensure role is PASSENGER for this app
      const requestData = {
        ...data,
        role: 'PASSENGER'
      };
  ...
  verifyOtp: async (data: VerifyOtpRequest): Promise<PhoneAuthResponse> => {
    try {
      ...
      if (authResponse.user && authResponse.user.role !== 'PASSENGER') {
  ...
  refreshToken: async (refreshToken: string): Promise<RefreshTokenResponse> => {
    try {
      ...
      throw new Error(getContextualErrorMessage(error, 'refresh token'));
    }
  },
  ...
  getProfile: async (): Promise<{ success: boolean; user: PhoneUser }> => {
    try {
      ...
      if (profileData.user && profileData.user.role !== 'PASSENGER') {
        throw new Error('Your account role has changed. Please contact support.');
      }

      return profileData;
    } catch (error: any) {
      throw new Error(getContextualErrorMessage(error, 'get profile'));
    }
  },
  ...
  updateProfile: async (data: { name: string }): Promise<{ success: boolean; message: string; user: PhoneUser }> => {
    try {
      ...
      throw new Error(getContextualErrorMessage(error, 'update profile'));
    }
  },
